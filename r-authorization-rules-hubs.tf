resource "azurerm_eventhub_authorization_rule" "listen" {
  ...
  name                = try(format("%s-listen", each.value.custom_name), var.use_caf_naming ? data.azurecaf_name.eventhub_auth_rule[format("%s.listen", each.value.hub_name)].result : "listen-default")
