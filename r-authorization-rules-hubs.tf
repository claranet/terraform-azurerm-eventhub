resource "azurerm_eventhub_authorization_rule" "listen" {
  ...
  name                = var.use_caf_naming ? azurecaf_name.eventhub_auth_rule[format("%s.%s", each.value.hub_name, each.value.rule)].result : "listen-default"
...
resource "azurerm_eventhub_authorization_rule" "send" {
  ...
  name                = var.use_caf_naming ? azurecaf_name.eventhub_auth_rule[format("%s.%s", each.value.hub_name, each.value.rule)].result : "send-default"
  namespace_name      = azurerm_eventhub_namespace.eventhub.name
  eventhub_name       = azurerm_eventhub.eventhub[each.value.hub_name].name
  resource_group_name = var.resource_group_name
...
resource "azurerm_eventhub_authorization_rule" "manage" {
  ...
  name                = var.use_caf_naming ? azurecaf_name.eventhub_auth_rule[format("%s.%s", each.value.hub_name, each.value.rule)].result : "manage-default"
  namespace_name      = azurerm_eventhub_namespace.eventhub.name
  eventhub_name       = azurerm_eventhub.eventhub[each.value.hub_name].name
  resource_group_name = var.resource_group_name
